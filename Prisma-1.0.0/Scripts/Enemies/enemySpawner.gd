...
func settings(enemy_scene):
	...
	if $"..".get_node("PauseTimer").time_left == 0:
		...
		if index != "kamikaze" and index != "upgrade":
			E.global_position.x = spawn_position["x"][index][randi() % spawn_position["x"][index].size()]
			E.global_position.y = rail_limits_checker(index)
			E.choice = E.global_position.y
			change_rail_counter(E.global_position.y, true)
